...
import { Package, Heart, ShoppingCart, Search, Filter, Star, Leaf } from 'lucide-react';
...
const MarketplacePage = () => {
    ...
    try {
      ...
      const [productsRes, materialsRes, categoriesRes] = await Promise.all([
        ...
        marketplaceService.materialListings.getAll(params).catch((err) => {
          console.log('Materials API error:', err.message, err.response?.status);
          return { data: { results: [] } };
        }),
        marketplaceService.categories.getAll().catch((err) => {
          console.log('Categories API error:', err.message, err.response?.status);
          return { data: [] };
        }),
      ]);
  ...
  const ProductCard = ({ item, type = 'product' }) => (
    <Card className="overflow-hidden hover:shadow-2xl transition-all duration-300 card-hover border-sage/20">
      <div className="relative h-48 bg-gradient-to-br from-cream/50 to-sage/10">
        ...
        >
          <Heart className="w-5 h-5" fill={item.is_favorited ? "#FFB823" : "none"} stroke={item.is_favorited ? "#FFB823" : "currentColor"} />
        </Button>
        {item.status === 'draft' && (
          <Badge className="absolute top-2 left-2 bg-orange text-white" variant="secondary">Draft</Badge>
        )}
      </div>
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <Badge variant="outline" className="border-sage/50 text-forest">{item.condition}</Badge>
          </div>
        </div>
      </CardContent>
  ...
  console.log('Rendering MarketplacePage - products:', products.length, 'materials:', materials.length, 'loading:', loading);
                ...
                <Button 
                  ...
                  onClick={() => {
                    if (!isAuthenticated) {
                      ...
                      toast({
                        title: 'Coming Soon',
                        description: 'Listing items feature will be available soon',
                      });
                    }
                  }}
                >
        ...
        <div className="container mx-auto px-4 py-12">
          <Tabs defaultValue="all" className="w-full">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-3 bg-cream/50">
              <TabsTrigger value="all" className="data-[state=active]:bg-forest data-[state=active]:text-white">All Items</TabsTrigger>
              <TabsTrigger value="products" className="data-[state=active]:bg-forest data-[state=active]:text-white">Products</TabsTrigger>
              <TabsTrigger value="materials" className="data-[state=active]:bg-forest data-[state=active]:text-white">Raw Materials</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="all" className="mt-8">
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold text-forest mb-2">No Items Available</h3>
                  <p className="text-muted-foreground mb-6">
                    The marketplace is currently empty. Check back soon or be the first to list an item!
                  </p>
                  <Button 
            ...
            <TabsContent value="products" className="mt-8">
              {loading ? (
                ...
                <div className="text-center py-12">
                  <Package className="w-16 h-16 text-sage/40 mx-auto mb-4" />
                  <h3 className="text-xl font-semibold text-forest mb-2">No Products Available</h3>
                  <p className="text-muted-foreground">No products found. Be the first to sell one!</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <TabsContent value="materials" className="mt-8">
              {loading ? (
                ...
                <div className="text-center py-12">
                  <Package className="w-16 h-16 text-sage/40 mx-auto mb-4" />
                  <h3 className="text-xl font-semibold text-forest mb-2">No Materials Available</h3>
                  <p className="text-muted-foreground">No raw materials found. List your recyclable materials!</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {materials.map((material) => (
                    <ProductCard key={material.id} item={material} type="material" />
                  ))}
                </div>
              )}
            </TabsContent>
